...
public class TableViewFactory {
	 ...
	 */
	public static void setTable(TableView table, Class voClazz) {	// 기존에 있던 테이블에 세팅해주는 테이블
	 ...
	 */
	public static TableView getTable(Class voClazz) {	// 보클래스 지정하고 원하는 테이블뷰리턴시켜 사용하는 테이블뷰
	 ...
	 */
	public static TableView getTable(String sql, Connection conn) {	// 데이터베이스 들어가면 대화가 가능하게 해줌

		TableView<Map> table = null;	// 키와 밸류를 이용하기 때문에 범용성이 높다
		TableColumn[] columns = null;	// 질의 결과가 몇개인지 몇개인지 알수 없기 때문에 배열로 놓음
		ResultSet rs = null;			// 테이블뷰를 만드는 건 select질의 하나만 있음 그러므로 무조건 리절트 셋이 오는것임
		ResultSetMetaData rsmd = null;	// 컬럼안의 메타데이터가 들어있는 것임
		int columnCount = 0;			// 몇개의 컬럼인지 표현
...
// 컬럼수 만큼 한바퀴 돌련서 ui용 컬럼을
			for (int i = 0; i < columnCount; i++) {
				columns[i] = new TableColumn<Map, String>(rsmd.getColumnName(i + 1));	// ui컬럼과db컬럼의 이름 매핑시켜줌(0번이 아니라 1번부터라서 +1시켜준거임)
				columns[i].setCellValueFactory(new MapValueFactory(rsmd.getColumnName(i + 1)));	// 셀값을 넣어주는 맵밸류공장 호출해서 이름 매핑(0번이 아니라 1번부터라서 +1시켜준거임)
// c[i].setMinWidth(130);
// Cell edit를 위한 전초작업
				columns[i].setCellFactory(TextFieldTableCell.forTableColumn());	// 셀 더블클릭해서 수정할수 있게 해주는 기능
//
			}
			// 옵저버블리스트 만드는 방법(특이)
...
// 맵형태로 만든다(컬럼이름을 키로 하고 해당값을 rs에서 읽어온것)
				for (int i = 0; i < columnCount; i++) {
					String value = rs.getString(i + 1);	// 범용성을 위해 String으로
					dataRow.put(rsmd.getColumnName(i + 1), value);	// 컬럼이름을 키로 잡음
				}
// 맵이 다만들어지면 옵저버블 리스터에 이 멥을 추가한다.
			...
			table.getColumns().setAll(columns);	// 겟컬럼스하면 옵저버블리스트에 담겨옴->배열로만든컬럼스를 셋올한건 셋팅시켜서 연결시켜주는것임
