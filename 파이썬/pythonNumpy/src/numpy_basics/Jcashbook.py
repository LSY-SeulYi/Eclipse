"""
    - 프로그램명 : Jcashbook 가계부
    - 작성자     : 정영환
    - 제목       : 일자, 수입/지출, 세부항목, 적요, 금액 등을 관리하는 가계부 프로그램
    - 설명       : 1) PyQt5 라이브러리를 사용해서 만든 윈도우용 가계부
                   2) sqlite3 라이브러리를 사용하여 .db 파일로 가계부 데이터 저장
                   3) 창 크기 고정 (가로 : 1000, 세로 : 740), 가변 창으로 하려니 어려워서...
                   4) 클래스 (=화면)
                      - excelOutDialog(QDialog)      : 엑셀 내보내기 화면
                      - delDataDialog(QDialog)       : 데이터 삭제하기 화면
                      - chartDialog(QDialog)         : 차트 화면
                      - setDialog(QDialog)           : 수입/지출 세부 항목 설정 화면
                      - helpDialog(QDialog)          : 도움말 화면
                      - InsertDialog(QDialog)        : 가계부 입력 화면 (수정/삭제도 같은 클래스 사용)
                      - JcashbookWindow(QMainWindow) : 메인 화면
                   5) 기능
                      ...
                      - 엑셀 불러오기 메뉴를 사용하여 엑셀 데이터를 Jcashbook 으로 가져올 수 있음
                      - 엑셀 내보내기 메뉴를 사용하여 Jcashbook 데이터를 엑셀 파일로 내보낼 수 있음
                      - 전체/년도/월 단위로 데이터를 삭제할 수 있음
                   6) 기타
                      - 편의상 PyQt5 모듈 import 시 "*" 사용
                      - 편의상 PyQt5 객체 앞에는 모두 "self" 를 붙임
    - 변경관리   : 1) v1.0, 2018-09-17 최초작성
"""

# [QT Designer 사용]
# 1. 일단 QT Designer로 *.ui라는 xml파일을 생성
# 2. pyuic5 툴로 ui 파일을 python 파일로 변환 (C:\Anaconda3\Lib\site-packages\PyQt5\uic)
#    pyuic5 -x JcashBook.ui -o JcashBook2.ui.py
# 3. QT Designer로 만들어진 소스를 그대로 사용하기 어려워서 참조 용도로만 사용

# [pyinstaller 사용]
# pyinstaller -h
# pyinstaller Jcashbook.py
# pyinstaller -F -w Jcashbook.py   -F : 최종 실행파일 하나만 생성, -w : 콘솔 창 없이 실행되도록 지정
# 최종 exe 파일 용량이 생각보다 크긴 한데 그래도 뭐...
...
# '파일' --> '엑셀 내보내기'
# excelOutMenu = QMenu('엑셀 내보내기', self)    # '새 파일' 메뉴 객체 생성
# excelOutMenu.setFont(QFont('돋움', 10))
# fileMenu.addMenu(excelOutMenu)                 # 'fileMenu' 메뉴에 'New' 메뉴 추가
# '파일' --> '엑셀 내보내기' --> '현재 화면 내보내기'/'1년 데이터 내보내기'
# currDataExcelOutAct = QAction('현재 데이터 엑셀 내보내기', self)     # '현재 데이터 엑셀 내보내기' 액션 생성
# excelOutMenu.addAction(currDataExcelOutAct)                          # 'fileNew' 메뉴에 '텍스트 문서' 액션 추가
# currDataExcelOutAct.triggered.connect(self.currDataExcelOutActTriggered)
# data = [random.random() for i in range(25)]
# plt.plot(data)
# plt.title('월별 수입/지출 현황 분석')
# plt.show()
# data1 = [random.random() for i in range(25)]
# data2 = [random.random() for i in range(25)]
# ax.plot(data_amt, data_month, 'r-')
# ax.bar(incMon, incAmt, width=0.3, align='center', label='수입')
# ax.bar(expMon, expAmt, width=0.3, align='center', label='지출')
...
class excelOutDialog(QDialog):
    ...
    def setupUI(self):
        ...
        self.radio2 = QRadioButton("Jcashbook 전체 데이터", self)
    ...
    def pushButton6Clicked(self):
        ...
        ws1["D1"] = "적요"
        ws1["E1"] = "금액"
        # 출력할 Jcashbook 데이터셋 만들기
        if self.radio1.isChecked():
            excelOutData = jcashbookWindow.selectMainDataResults
        elif self.radio2.isChecked():
            conn = sqlite3.connect('Jcashbook.db')
            with conn:
                cursor = conn.cursor()
...
class chart2Dialog(QDialog):
    ...
    def setupUI(self):
        ...
        # 차트 클래스 호출
        self.chart2 = Plot2Canvas(self, width=7.5, height=5, whereStr1=self.comboBox1.currentText()[0:4], whereStr2=self.comboBox2.currentText()[0:2])
        self.chart2.move(15, 15)
    ...
    # 년도 선택 시
    def comboBox1Activated(self):
        self.chart2.plot(self.comboBox1.currentText()[0:4], self.comboBox2.currentText()[0:2], self.comboBox3.currentText())
...
▣ Jcashbook 설명
   ...
   - 카드/통장 등과 연관된 기능이 없으며 향후 버전업 시 해당 기능이 추가될 수 있습니다.
   - 복잡하고 사용하기 어려운 가계부가 아닌 심플한 기본 기능만을 원하는 분께 적합합니다.
...
▣ Jcashbook 부가 기능 (파일 메뉴)
   1. 엑셀 불러오기
      - 엑셀 데이터를 Jcashbook 으로 가져올 수 있습니다.
      - 엑셀 내보내기 시 만들어지는 양식을 사용하세요.
   2. 엑셀 내보내기
      - Jcashbook 데이터를 엑셀 파일로 내보낼 수 있습니다.
      - 가계부 실행파일이 존재하는 위치에 'Jcashbook_Data.xlsx' 파일이 만들어집니다.
   3. 데이터 삭제하기
      - 전체/년도/월 단위로 데이터를 삭제할 수 있습니다.
      
▣ Jcashbook 차트 기능 (차트 메뉴)
   1. 월별 수입/지출 현황 분석
...
# - 엑셀 불러오기 메뉴를 사용하여 엑셀 데이터를 Jcashbook 으로 가져올 수 있음
# - 엑셀 내보내기 메뉴를 사용하여 Jcashbook 데이터를 엑셀 파일로 내보낼 수 있음
# - 전체/년도/월 단위로 데이터를 삭제할 수 있음""", self)
...
class InsertDialog(QDialog):
    def __init__(self, mainWindowLeft, mainWindowTop, dmlCls, tableWidget1RowData):
        ...
        self.setGeometry(dialogLeft, dialogTop, dialogWidth, dialogHeight)
        self.setFixedSize(dialogWidth, dialogHeight)
        self.setWindowTitle("Jcashbook 데이터 입력")
        self.setModal(True)
    ...
    def closeEvent(self, evnt):
        jcashbookWindow.selectMainData()
...
class JcashbookWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Jcashbook v1.0")
    ...
    def setupUI(self):
        ...
        statusBar.showMessage("Jcashbook 가계부 프로그램입니다.")
        # self.statusBar().showMessage("Jcashbook 가계부 프로그램입니다.")
    ...
    def tableWidget1DoubleClicked(self, row, column):
        ...
        sg = self.geometry()
        insertDialog = InsertDialog(sg.left(), sg.top(), '1', tuple(tableWidget1RowData))    # dmlCls --> 1 : 수정/삭제, 2 : 입력
        insertDialog.exec_()
    ...
    def excelInActTriggered(self):
        ...
        with conn:
            cursor = conn.cursor()
            cursor.executemany("INSERT INTO JCASHBOOK_MAIN_DATA (BAS_DT, INC_EXP_CLS, INC_EXP_DTL_CLS, NOTE, AMT) VALUES (?,?,?,?,?)", excelIndata)
