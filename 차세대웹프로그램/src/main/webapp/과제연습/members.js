...
function checkId() {
	...
	if (id.value === "") {
		...
		error[0].innerHTML = "아이디가 입력되었습니다.";
...
function checkPw() {
	...
	if (pw1.value === "") {
		...
		pwMsg.innerHTML = "사용불가능한 비밀번호입니다.";
...
function comparePw() {
	...
	if (pw2.value === "") {
		error[2].innerHTML = "비밀번호 확인을 입력하세요";
		error[2].style.display = "block";
	}
}
...
function checkAge() {
	if (Number(yy.value) < 1900) {
		error[4].innerHTML = "잘못 입력된 정보입니다.";
		error[4].style.display = "block";
...
function checkPhoneNum() {
	...
	if (mobile.value === "") {
		error[7].innerHTML = "휴대폰 번호를 입력하세요.";
